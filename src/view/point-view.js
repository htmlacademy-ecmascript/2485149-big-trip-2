...
function createPointTemplate(point, destination, offers) {
  ...
  const startDate = new Date(dateFrom).toLocaleString('en-GB', {
    hour: '2-digit',
    minute: '2-digit',
  });
  const endDate = new Date(dateTo).toLocaleString('en-GB', {
    hour: '2-digit',
    minute: '2-digit',
  });
  const duration = Math.round(
    (new Date(dateTo) - new Date(dateFrom)) / (1000 * 60)
  );
  ...
  return `
    <li class="trip-events__item">
      <div class="event">
        <time class="event__date" datetime="${dateFrom}">${new Date(
  dateFrom
).toLocaleDateString('en-GB', { month: 'short', day: 'numeric' })}</time>
...
export default class PointView extends AbstractView {
  ...
  .addEventListener('click', this.onOpenEditButtonClick);
  }
  ...
  togglePoint() {
    this.element.classList.toggle('event--expanded');
  }
}
